...
![极其庞大的反混淆前的基本块链接图](./极其庞大的反混淆前的基本块链接图.png)

瞧瞧这，实在是太大了，所以这里我们只截取几张局部的来看：

![反混淆前的基本块链接图1](./反混淆前的基本块链接图1.png)  

![反混淆前的基本块链接图2](./反混淆前的基本块链接图2.png)  
...
![单独迭代合并基本块反混淆大局](./单独迭代合并基本块反混淆大局.png)

现在看起来至少比上面原始的基本块链接图精小了不少。

![单独迭代合并基本块反混淆1](./单独迭代合并基本块反混淆1.png)

![单独迭代合并基本块反混淆2](./单独迭代合并基本块反混淆2.png)
...
![迭代合并基本块和OP反混淆大局](./迭代合并基本块和OP反混淆大局.png)

又变得精细了，让我们看看之前举的基本块例子 `0x14043bf81`

![迭代合并基本块和OP反混淆1](./迭代合并基本块和OP反混淆1.png)

很好，这下基本块 `0x14043bf81: 88` 已经是打回原形了，它已经没有后继基本块了。

![迭代合并基本块和OP反混淆1-x64dbg](./迭代合并基本块和OP反混淆1-x64dbg.png)

![迭代合并基本块和OP反混淆1-x64dbg2](./迭代合并基本块和OP反混淆1-x64dbg2.png)

![迭代合并基本块和OP反混淆1-x64dbg3](./迭代合并基本块和OP反混淆1-x64dbg3.png)

到此，我们又成功刺杀了永真永假型不透明谓词混淆，以及基于它所构造的代码乱序混淆。

![不透明谓词证明情况](./不透明谓词证明情况.png)
...
![极其庞大的反混淆后的基本块链接图](./极其庞大的反混淆后的基本块链接图.png)

到此，我们又成功刺杀了间接跳转混淆，以及基于它所构造的代码乱序混淆。

![间接跳转混淆证明情况](./间接跳转混淆证明情况.png)
...
![内存操作数常量隐藏证明情况](./内存操作数常量隐藏证明情况.png)
...
```
![反混淆脚本-内存操作数常量隐藏](./反混淆脚本-内存操作数常量隐藏.png)

![反OP脚本执行](./反OP脚本执行.png)

![反MEM脚本执行](./反MEM脚本执行.png)

然后我们输入 `3735928559` 让他正确弹出信息框


![反混淆后正常运行](./反混淆后正常运行.png)

![反混淆后正常运行2](./反混淆后正常运行2.png)
